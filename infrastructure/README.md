...
The 2026-08-16 staging activation used [Deploy staging #31941240921](https://github.com/SEQRETE-TUK/SEQRET_BE/actions/runs/31941240921) for the first split rollout and [Deploy staging #31941666124](https://github.com/SEQRETE-TUK/SEQRET_BE/actions/runs/31941666124) for the second. The resulting stable API revision `seqret-stg-api-00027-mt9`, rollback revision `seqret-stg-api-00026-t2x`, worker revision `seqret-stg-worker-00016-hgb`, migration job, and relay job each reference only their dedicated Secret. All 35 application tables are owned by `seqret_migration`; API, worker, and relay have `SELECT`/`INSERT`/`UPDATE` and no `DELETE`, while recovery has `SELECT` only and enforced read-only transactions. The full media canary completed analysis `771d2926-e29b-4b53-af70-1d21e9f7dd73`, imported one scope from six detections, and published each capture and analysis event once. Runtime DDL, audit mutation, and recovery writes were rejected. The legacy application Secret versions are disabled after credential rotation, and [PITR recovery #31942096659](https://github.com/SEQRETE-TUK/SEQRET_BE/actions/runs/31942096659) verified the split recovery credential, current schema, marker row, and exact clone cleanup.

`MEDIA_BUCKET_NAME` must already exist as a private bucket. Terraform gives the API service account bucket-scoped object create/view access and self-scoped URL-signing permission, and gives only the private worker object read/delete access; it does not create or make the bucket public. After the migration-approved plan is applied, the staging workflow replaces bucket CORS with one rule for the exact `FRONTEND_ORIGIN`, `GET`/`HEAD`/`PUT`, `Content-Type`, and generation/hash response headers, then fails closed unless the read-back matches exactly.
...
`Deploy staging` runs automatically on every push to `main` and remains available for manual dispatch from `main`. Before requesting a Google Cloud identity or changing infrastructure, it calls the reusable `CI` workflow from the same commit and requires repository-integrity, Python, Terraform, and container checks to pass. The deployment then checks out the triggering commit, requires it to still match the current `origin/main` commit, and:
...
This deployment-local quality gate also protects an accidental direct push, but it does not replace repository branch protection. Configure `main` to require pull requests and the four CI checks as a separate GitHub repository policy.
...
Terraform enables Container Scanning and the staging deploy waits up to 15 minutes for package vulnerability analysis of the exact published digest before running migration or changing a service. A missing or failed scan, inactive continuous analysis, or any CRITICAL/HIGH finding with an available fix blocks deployment. A CRITICAL/HIGH finding without an available fix also blocks unless [`artifact_vulnerability_exceptions.json`](artifact_vulnerability_exceptions.json) contains an exact severity, vulnerability ID, and package exception with an unexpired date and this repository's tracking issue. A stale exception blocks too, so resolved findings must be removed rather than becoming permanent allow-list entries. The current no-fix exceptions expire on 2026-09-15; reassess the upstream Debian packages and pinned base image before that date.

Artifact Analysis charges USD 0.26 for each newly scanned container digest as of 2026-08-16. At 30 distinct digests per month this is USD 7.80, and at 60 it is USD 15.60, before tax or credits; recheck the [official pricing](https://cloud.google.com/artifact-analysis/pricing) before changing build frequency or scan scope. Existing images are not automatically scanned when the API is enabled, so only a `FINISHED_SUCCESS` result for the deployment digest is evidence.

Repository cleanup remains in dry-run. On 2026-08-17 the repository held 29 versions and about 1.43 GB, fewer than the keep-most-recent 50 boundary, and both cleanup candidate logs and actual `BatchDeleteVersions` audit records for the prior 30 days were zero. The current and rollback digests both exist and are protected by the keep policy. The repository was created on 2026-08-12, so 2026-11-11 is only the earliest date an image can satisfy the 90-day delete condition; a keep policy still overrides a matching delete policy. After that date, inspect the dry-run audit log for real candidates, prove the active and rollback digests are excluded, and only then set `cleanup_policy_dry_run=false`; verify the exact deleted versions afterward. Follow the [cleanup policy overview](https://docs.cloud.google.com/artifact-registry/docs/repositories/cleanup-policy-overview) and [cleanup audit procedure](https://docs.cloud.google.com/artifact-registry/docs/repositories/cleanup-policy).

The 2026-08-16 activation evidence is [Deploy staging #31944131531](https://github.com/SEQRETE-TUK/SEQRET_BE/actions/runs/31944131531). The OIDC deployment identity read the newly published digest through Artifact Analysis, observed `FINISHED_SUCCESS` with continuous analysis `ACTIVE`, and passed four no-fix CRITICAL plus five no-fix HIGH findings through the nine exact unexpired exceptions before migration began. Migration, Terraform apply, relay verification, revision readiness, public database readiness, 10 percent canary, and promotion then succeeded. The resulting repository held 27 versions; the active and rollback digests were present and the cleanup dry-run audit count remained zero.

The 2026-08-17 current-main evidence is [Deploy staging #31988583224](https://github.com/SEQRETE-TUK/SEQRET_BE/actions/runs/31988583224). Source `1b2d8de3596c398fe706da0d0b9f6f24ff053e20` produced digest `sha256:6047b48da196a2f93017ef22585e4f1a57585bae18f40e4ad26c888590538a21`; the same vulnerability gate, migration, Terraform, relay, readiness, canary, and promotion checks passed. API revision `seqret-stg-api-00030-w9z` and worker revision `seqret-stg-worker-00019-lrt` use the digest, while `seqret-stg-api-00029-755` remains the rollback revision. The repository held 29 versions and about 1.43 GB, and no cleanup candidate or actual deletion audit record existed.
