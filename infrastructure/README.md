...
Repository cleanup remains in dry-run. On 2026-08-16 the repository held 26 versions, fewer than the keep-most-recent 50 boundary, and the dry-run `BatchDeleteVersions` audit count was zero. The current and rollback digests both exist and are protected by the keep policy. The repository was created on 2026-08-12, so 2026-11-11 is only the earliest date an image can satisfy the 90-day delete condition; a keep policy still overrides a matching delete policy. After that date, inspect the dry-run audit log for real candidates, prove the active and rollback digests are excluded, and only then set `cleanup_policy_dry_run=false`; verify the exact deleted versions afterward. Follow the [cleanup policy overview](https://docs.cloud.google.com/artifact-registry/docs/repositories/cleanup-policy-overview) and [cleanup audit procedure](https://docs.cloud.google.com/artifact-registry/docs/repositories/cleanup-policy).
