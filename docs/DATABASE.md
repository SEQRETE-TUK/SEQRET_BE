...
- `workspace_account`는 역할 하나의 서버 작업공간을, `workspace_membership`은 그 계정에 연결된 작업 participant를 보존한다. 한 participant는 active 계정 하나에만 연결된다. `workspace_session`은 30일 만료·철회 상태, SHA-256 cookie hash와 CSRF token을 저장하며 cookie 원문은 저장하지 않는다.
- `workspace_contact_point`는 account·`EMAIL|SMS|KAKAO`별 최대 한 개의 명시적 수신 동의 destination과 활성 상태를 보존한다. API는 원문을 반환하지 않지만 실제 provider 전달에 필요한 destination은 DB에 저장하므로 접근권한·로그·backup을 개인정보로 취급한다.
...
- `notification_delivery`는 in-app 이력과 외부 채널 snapshot을 함께 저장한다. 외부 row는 destination, 다음 시도, lease token·만료, provider request ID를 보존하며 `(event_id, recipient_participant_id, channel)`이 유일하다. provider secret과 메시지 원문은 저장하지 않는다.
