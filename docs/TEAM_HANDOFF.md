...
- **DB 역할:** API, migration, Outbox relay와 recovery가 현재 같은 `seqret` 역할과 `DATABASE_URL_SECRET_ID`를 사용한다. staging 실조회에서 이 역할은 superuser가 아니지만 `CREATEDB`·`CREATEROLE`을 보유한다. `audit_event` mutation trigger는 일반 DML도 거부하지만 DDL 우회가 가능하므로 tamper-proof 권한 경계로 간주하지 않는다. 별도 runtime·migration·recovery 사용자, grant와 Secret ID를 함께 준비하기 전에는 Terraform secret만 나누지 않는다.
- **DB 연결 경보:** staging PostgreSQL은 `max_connections=25`, 예약 3개, 일반 실행 최대 10개, rollout 중 보수적 최대 18개로 확정했다. `num_backends`를 instance의 모든 database에 걸쳐 합산해 18 초과가 2분 지속되면 WARNING을 보낸다.
- **Artifact Registry:** 90일 초과 삭제 후보와 최신 50개 보존 정책은 계속 dry-run이다. provider cleanup audit log를 검토한 뒤에만 실제 삭제로 전환한다.
- **B runtime:** Cloud Tasks queue·OIDC private worker, media validation과 실제 Vertex 분석을 staging에서 확인했다. worker service account에는 `roles/aiplatform.user`가 있고 Vertex AI API와 `asia-northeast3` location 설정도 적용됐다. [#103](https://github.com/SEQRETE-TUK/SEQRET_BE/pull/103)은 raw body·GCS URI 없이 `analysis_stage`, `provider_status`, `error_kind`, `retryable`을 기록하고 retryable 실패를 최대 5회 503 재전달로 연결했다. 다만 B run이 FAILED로 커밋된 직후 reopen 전에 프로세스가 중단되면 재전달을 non-retryable conflict로 잘못 끝내는 crash-window가 남아 있어 [#93](https://github.com/SEQRETE-TUK/SEQRET_BE/issues/93)은 닫지 않는다.
- **INT-01 runtime:** `8b97591` staging에서 합성 PNG의 signed GCS PUT → Cloud Tasks validation `READY` → 실제 Vertex 분석 `completed` → A 범위 초안 4개 품목 import를 검증했다. A의 제출·durable dispatch·worker 결과 import와 Vertex runtime wiring 성공 증적은 완료됐으며, 남은 [#93](https://github.com/SEQRETE-TUK/SEQRET_BE/issues/93) 범위는 위 retry crash-window다.
...
현재 staging application은 `8b975918aa0df68b5bfe1e9321a752423c809480`, schema head는 `int_04_0001`이며 위 INT-06 배포 증적과 일치한다. `72c4840`에서 수정한 recovery workflow는 위 PITR drill로 재검증했다.
