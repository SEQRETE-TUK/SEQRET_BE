...
- `GET /api/v1/move-jobs/{job_id}/capture-sessions/{capture_session_id}/analysis`는 촬영 소유자에게 `pending|dispatching|queued|running|completed|failed`, 선택적 `scope_version_id`, provider-neutral `failure_code`·`retryable`과 `failure_stage`·선택적 `provider_status`·`failure_detail_code`를 반환한다. object key, provider task·모델 응답·provider 오류 원문은 노출하지 않는다.
...
- Vertex v2 prompt에는 주소·사용자·UUID 대신 입력 순번, 출·도착 종류와 익명 위치·방 group만 전달하고 `temperature=0`으로 구조화 출력을 요청한다. provider는 최종 가격이나 ID를 결정하지 않는다. 수량·단위가 하나라도 없으면 둘 다 비운 검수 품목으로, 상태와 값이 어긋난 층수·운반거리는 `unknown` 검수 필드로 정규화하고 중복 검수 필드는 제거한다. 그 밖의 엄격한 schema 위반은 `parse / schema_validation`으로 종료한다.
